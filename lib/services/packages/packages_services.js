...
import { packages_endpoint ,
  package_bookings_endpoint ,
   package_cancelPenalty_endpoint ,
    packages_bookingCard_endpoint,
    package_Booking_pricing
...
const packagesService = {
    ...
    getPackagesReservationData: async (params) => {
      ...
      pricingBookingCard: async (id) => {
        try {
          const response = await apiService.get(
             `${package_Booking_pricing}/${id}`,
            commonHeader
          );
          return response;
        } catch (error) {
